/// Capability is a pattern that allows authorizing actions with an object. 
/// One of the most common capabilities is TreasuryCap (defined in sui::coin).
module examples::item {
    ...
    /// Module initializer is cal,led once on module publish.
    ...
    /// the first argument. Hence only owner of the `AdminCap` can perform this action.
    public fun create_and_send(
        _: &AdminCap, name: vector<u8>, to: address, ctx: &mut TxContext
    ) {
        transfer::transfer(Item {
            id: object::new(ctx),
            name: string::utf(name)
        }, to)
    }
}